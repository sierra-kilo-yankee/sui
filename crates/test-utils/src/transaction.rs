...
use sui_types::messages::GAS_UNIT_FOR_GENERIC;
use sui_types::messages::GAS_UNIT_FOR_PUBLISH;
...
pub async fn publish_package_with_wallet(
    ...
    let transaction = {
        let data = client
            ...
            .publish(
                ...
                dep_ids,
                None,
                GAS_UNIT_FOR_PUBLISH * gas_price * 10,
            )
            .await
            .unwrap();
    ...
    (
        effects
            ...
            .reference
            .to_object_ref(),
        effects.gas_object().reference.to_object_ref(),
        resp.digest,
    )
}
...
> {
    ...
    let object_to_send: ObjectID = object_refs
        .iter()
        .find(|resp| resp.data.as_ref().unwrap().bcs.as_ref().unwrap().try_as_move().map(|m| m.has_public_transfer).unwrap_or(false))
        .unwrap()
        .data
        .as_ref()
        .unwrap()
