...
impl Env {
    ...
    async fn setup_remote_env(
        ...
        // TODO: investigate if getting objects can be used, current issue is sequence
        // will be wrong on the second attempt
        ...
        // let primary_gas = objects_by_owner
        ...
        // let pay_coin = objects_by_owner
        //     .get(&Owner::AddressOwner(primary_gas_owner_addr.into()))
        //     .expect("primary gas owner not found")
        //     .iter()
        //     .filter(|gas_object| *gas_object.id() != *primary_gas.id())
        //     .choose(&mut rand::thread_rng())
        //     .context("Failed to choose a random pay coin")?
        //     .clone();

        // let pay_coins = all_gas_objects
        //     .iter()
        //     .try_fold(Vec::new(), |mut acc, gas_object| {
        //         if gas_object.1 == primary_gas {
        //             Ok(acc)
        //         } else {
        //             let pay_coin = (
        //                 gas_object.1.compute_object_reference(),
        //                 gas_object.1.owner.get_owner_address().map_err(|e| {
        //                     anyhow!(format!("Failed to get owner address for pay coin"))
        //                 })?,
        //                 keypair.clone(),
        //             );
        //             acc.push(pay_coin);
        ...
        // Require the use of fullnode to query for gas objects.
        if fullnode_rpc_urls.is_empty() {
        ...
        let pay_coin_objs = gas_objects
            .iter()
            .filter(|(balance, gas_object)| {
